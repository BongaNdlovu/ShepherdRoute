...
declare
  ...
  computed_urgency public.urgency_level := 'medium';
begin
  ...
  if 'pastoral_visit' = any(p_interests)
    or 'baptism' = any(p_interests)
    or coalesce(p_message, '') ~* '(urgent|pastor|visit|baptism|struggling|counselling)' then
    computed_urgency := 'high';
  elsif 'health' = any(p_interests) or 'cooking_class' = any(p_interests) then
  ...
  values (
    ...
    now(),
    'public_form',
    jsonb_build_object('classification_version', null, 'ready_for_ai', true)
  )
