...
export function ContactMethodConsent({
  ...
  return (
    <div className="mobile-safe-container space-y-4">
      <div className="mobile-safe-container rounded-lg border bg-muted p-4">
        <h3 className="mobile-safe-text font-semibold text-slate-900">Contact permission</h3>
        <p className="mobile-safe-text mt-1 text-sm text-slate-600">
        ...
        <div className="mobile-safe-container mt-3 space-y-2">
          {methods.map((method) => (
            <label key={method} className="mobile-safe-container flex items-center gap-2 rounded-md border bg-white px-3 py-2 text-sm font-semibold break-words">
              <input type="checkbox" name="preferred_contact_methods" value={method} className="h-4 w-4 shrink-0" />
              <span className="mobile-safe-text w-full min-w-0">{contactMethodLabels[method]}</span>
            </label>
          ))}
        </div>
        {errors?.preferred_contact_methods ? (
          <p className="mobile-safe-text mt-2 text-xs text-rose-600">{errors.preferred_contact_methods}</p>
        ) : null}
      </div>
      <div className="mobile-safe-container rounded-lg border bg-muted p-4">
        <h3 className="mobile-safe-text font-semibold text-slate-900">Consent statement</h3>
        <p className="mobile-safe-text mt-1 text-sm text-slate-600">
