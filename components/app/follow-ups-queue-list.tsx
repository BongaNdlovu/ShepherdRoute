  ...
  return (
    <div className="mt-5 overflow-hidden rounded-lg border">
      ...
      <div className="divide-y">
        {items.map((item) => {
          const hasWhatsAppDraft = Boolean(item.suggested_message?.message_text.trim());
            ...
            >
              ...
              <div className={cn("grid", compactLists ? "gap-1.5" : "gap-2")}>
                <form action={openSuggestedWhatsappAction}>
                  ...
                  <Button type="submit" variant="success" size="sm" className="w-full" disabled={item.contact.do_not_contact || !hasWhatsAppDraft}>
                    <MessageCircle className="h-4 w-4" />
                    {item.contact.do_not_contact ? "Opted out" : !hasWhatsAppDraft ? "No WhatsApp draft" : item.suggested_message?.opened_at ? "Open WhatsApp again" : "Open WhatsApp"}
                  </Button>
                </form>
