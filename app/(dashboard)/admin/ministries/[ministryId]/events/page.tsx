...
export const metadata = {
  title: "Owner Ministry Events"
};

export default async function OwnerMinistryEventsPage({
  params,
  searchParams
}: {
  ...
  const [ministry, eventsPage] = await Promise.all([
    getOwnerChurchDetail(ministryId),
    getOwnerChurchEventsPage(ministryId, paramsQuery)
  ]);
  ...
  return (
    <section className="space-y-4">
      <header className="rounded-lg border bg-white/10 p-5 shadow-sm">
        ...
        <h2 className="mt-1 text-2xl font-black tracking-tight">{ministry.name} - Events</h2>
      </header>

      <OwnerAdminTabs workspaceId={ministry.id} basePath="/admin/ministries" active="events" />

      <Card>
        <CardHeader>
          <CardTitle>Events</CardTitle>
          <CardDescription>Registration pages and QR codes for this ministry.</CardDescription>
        </CardHeader>
