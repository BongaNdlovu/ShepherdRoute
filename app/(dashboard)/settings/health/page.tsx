    ...
    <section className="space-y-4">
      <header className="rounded-lg border bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-2xl font-black tracking-tight">Setup health</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Confirm the production plumbing ShepherdRoute needs before inviting a church team.
            </p>
          </div>
          <Badge variant={failed ? "danger" : warned ? "warning" : "success"}>
        ...
        <Card>
          ...
          <CardContent className="grid gap-3">
            {checks.map((check) => {
              ...
              return (
                <div key={check.name} className="rounded-lg border bg-white p-4">
                  ...
                  {check.status !== "pass" && check.action ? (
                    <p className="mt-3 rounded-md bg-muted p-3 text-sm text-muted-foreground">{check.action}</p>
                  ) : null}
                </div>
              );
            })}
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm text-muted-foreground">
              ...
              <p className="rounded-md bg-muted p-3">Repo docs: docs/deployment.md and docs/observability.md.</p>
            </CardContent>
          </Card>
        </div>
      </div>
